...
const minimumDeviation = function(nums) {
    ...
    for(let n of nums) q.enqueue(n % 2 ? n * 2 : n);
    
    let ans = q.front().element - q.back().element;
    while(q.front().element % 2 === 0) {a
        q.enqueue(q.dequeue().element / 2);
        ans = Math.min(ans, q.front().element - q.back().element);
    }
