...
const StreamChecker = function(words) {
    this.trie = new ReversedFindTrie();
    this.current = '';

    for(let word of words) this.trie.insert(word.split('').reverse().join(''));
};

// query :: String -> Boolean
StreamChecker.prototype.query = function(letter) {
    this.current = this.current + letter;
    const trieWord = this.trie.find(this.current);

    return trieWord
};

// ReversedFindTrie :: () -> Object
function ReversedFindTrie() {
    this.root = { children: {} };
    ...
    this.find = function(word) {
        ...
        for(let i = word.length - 1; i >= 0; i--) {
            ...
            else if(currentNode.children[key].endWord) return true;
