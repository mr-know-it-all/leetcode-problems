...
// furthestBuilding :: ([Number], Number, Number) -> Number
const furthestBuilding = (heights, bricks, ladders) => {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if(diff > 0) {
        if (ladders > 0) {
          // we have unused ladders
          pq.enqueue(diff);
          ladders--;
        } else if (pq.front() && diff > pq.front().element) {
          // we have a diff larger than a the smallest diff that used a ladder
          // e.g:
          // diff = 8
          // pq = 4 -> 7 -> 9
          // step 1: pq = 4 -> 7 -> 8 -> 9
